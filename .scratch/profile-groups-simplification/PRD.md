...
35. As a Display Recall user, I do not want raw displayplacer command editing in the main Profile UI, so that advanced implementation details stay out of the daily path.
36. As a Display Recall user, I want import/export data to preserve existing profile fields, notes, commands, and display metadata, so that hidden UI does not destroy data.
37. As a Display Recall user, I want old imported or edited-command profiles to retain their safety behavior, so that simplifying the UI does not make applying risky profiles unsafe.
38. As a Display Recall user, I want fold state to reset predictably when opening the window, so that the current setup is always easy to find.
39. As a Display Recall user, I do not want fold state persisted, so that the app avoids surprising stale navigation state.
40. As a Display Recall user, I want generated profile names and generated display setup group names to use the current app language, so that new objects fit my language.
41. As a Display Recall user, I want existing profile names and group names not to change when switching language, so that my edited labels remain stable.
42. As a maintainer, I want display setup grouping extracted into a testable module, so that UI simplification is driven by stable behavior.
43. As a maintainer, I want profile storage migration tested, so that existing users' profiles are grouped without data loss.
44. As a maintainer, I want automatic apply switch behavior tested independently of SwiftUI, so that the single-choice rule stays correct.
45. As a maintainer, I want display setup group visibility rules tested independently of SwiftUI, so that current, non-current, empty, and non-empty groups render predictably.
46. As a maintainer, I want profile deletion behavior to continue using existing cleanup rules, so that automatic rules, shortcut bindings, and logs remain consistent.
47. As a maintainer, I want display setup group deletion tested independently of SwiftUI, so that deleting a group has predictable cascade behavior.
...
- Deleting the last profile in a group does not automatically delete the display setup group entity; only Delete Display Setup Group removes the group.
- Delete Display Setup Group uses native confirmation with concise copy.
- Non-empty group delete confirmation states the number of configurations that will also be deleted.
- Delete Display Setup Group records one group-level activity log entry instead of one profile-deleted entry per deleted configuration.
- The group delete log entry includes the display setup group name, deleted configuration count, and deleted configuration identifiers/names in diagnostics metadata.
...
- Display setup group actions: preserve rename and add delete with cascade cleanup for grouped configurations.
...
- Add delete tests that deleting the last profile does not automatically delete the group entity.
...
- Add display setup group delete tests for empty groups, non-empty groups, current groups, automatic apply rule cleanup, shortcut cleanup, and activity logging.
